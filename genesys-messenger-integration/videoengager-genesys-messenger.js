...
/**
  ...
  constructor ({ TENANT_ID, veUrl, customAttributes, autoAccept, audioOnly } = {}) {
     ...
     * @private
     */
    // Merge any buffered context set before instance existed
    this.customAttributes = {
      ...(window.__ctxBuffer || {}),
      ...(customAttributes || {})
    };
    window.__ctxBuffer = undefined;    // Call the function to inject the styles
    injectStyles();
...
async function sendStartVideoSessionMessage (interactionId) {
  try {
    const baseAttrs = (window.videoEngagerInstance && window.videoEngagerInstance.customAttributes) || {};
