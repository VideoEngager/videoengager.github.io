...
function registerUILogic () {
  ...
  function applyResponsiveBehavior () {
    ...
    if (windowElement) {
      if (isMobile()) {
        ...
        // Ensure the window stays within the screen bounds after resize
        const rect = windowElement.getBoundingClientRect();
        if (rect.right > window.innerWidth) {
          originalX = window.innerWidth - parseFloat(originalWidth);
        }
        if (rect.bottom > window.innerHeight) {
          originalY = window.innerHeight - parseFloat(originalHeight);
        }
        windowElement.style.transform = `translate(${originalX}px, ${originalY}px)`;
        windowElement.setAttribute('data-x', originalX);
        windowElement.setAttribute('data-y', originalY);
      }
    }
  }
  ...
  window.interact('.window').draggable({
    inertia: true,
    modifiers: [
      window.interact.modifiers.restrictRect({
        restriction: 'parent', // Restrict dragging within parent element
        endOnly: true
      })
    ],
    listeners: {
      move (event) {
        ...
        // Ensure the window doesn't go out of screen bounds
